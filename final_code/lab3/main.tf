...
# ---------- LAB 3 (TRANSIT GATEWAY FEDERATION TO CLOUD WAN) ----------

# TGW - Cloud WAN peering (both AWS Regions)
...
# AWS Transit Gateway Policy Tables (both AWS Regions)
...
resource "aws_ec2_transit_gateway_policy_table" "stockholm_tgw_policy_table" {
  ...
  tags = {
    Name = "tgw-policy-table-eu-north-1-${var.identifier}"
  }
}

# Transit Gateway Policy Table association to the TGW - CloudWAN peering attachment (both AWS Regions)
...
# Transit Gateway Route Table Cloud WAN Attachment (both AWS Regions)
resource "aws_networkmanager_transit_gateway_route_table_attachment" "oregon_cwan_tgw_rt_attachment" {
  ...
  tags = {
    Name = "${each.key}-us-west-2-tgw-rt-attachment"
    env  = each.key
  }
...
resource "aws_networkmanager_transit_gateway_route_table_attachment" "stockholm_cwan_tgw_rt_attachment" {
  ...
  tags = {
    Name = "${each.key}-eu-north-1-tgw-rt-attachment"
    env  = each.key
  }
